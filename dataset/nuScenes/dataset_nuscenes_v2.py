...
class Nuscenes(data.Dataset):
    ...
    def labelMapping(self, sem_label):
        sem_label = self.general_to_segmentation_index(sem_label) #(numpoint,1)       
        assert sem_label.shape[-1] == 1
        sem_label = sem_label[:, 0] #(numpoint)       
        return sem_label

    def getFilename(self, index):
        lidarseg_path =self.label_filenames[index]
        lidar_seg = lidarseg_path.split('_')
        lidar_seg1 = lidar_seg[0].split('/')           
        return lidar_seg1[2]
    
    def save_pred_pointcloud2csv(self,p_xyz,output3d_softmax,save_path):
        output3d_softmax = torch.argmax(output3d_softmax, dim=1)
        output3d_softmax = output3d_softmax.squeeze(dim=1)       
        predict_result = torch.cat((p_xyz,output3d_softmax),dim=1)
        predict_result_np = predict_result.cpu().detach().numpy()
        np.savetxt(save_path, predict_result_np, delimiter=',', fmt='%f')
        return 1
    
    def save_ground_pointcloud2csv(self,p_xyz,labels3d,save_path):       
        ground_truth = labels3d.squeeze(dim=1)       
        ground_truth = torch.cat((p_xyz,ground_truth),dim=1)
        ground_truth_np = ground_truth.cpu().detach().numpy()
        np.savetxt(save_path, ground_truth_np, delimiter=',', fmt='%f')
        return 1
