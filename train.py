            ...
            if (mode == 'Validation' and save_results_path is not None):
                ...
                if self.settings.dataset == 'nuScenes':
                    ...
                    if not os.path.isdir(grtruth_path):
                        os.makedirs(grtruth_path)
                    pred_result_path = os.path.join(pred_path, '{}_lidarseg.bin'.format(lidar_token))
                    pred_result_path_csv = os.path.join(pred_path, '{}_lidarseg.csv'.format(lidar_token))
                    grtruth_path_csv = os.path.join(grtruth_path, '{}_lidarseg.csv'.format(lidar_token))
                    pred_np.tofile(pred_result_path)
                    store_pred2csv = nu_dataset.save_pred_pointcloud2csv(pxyz,output3d_softmax,pred_result_path_csv)
                    store_ground2csv = nu_dataset.save_ground_pointcloud2csv(pxyz,labels3d,grtruth_path_csv)
